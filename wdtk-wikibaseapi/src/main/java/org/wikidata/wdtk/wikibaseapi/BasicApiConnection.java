...
public class BasicApiConnection extends ApiConnection {
     ...
     * site.
     *
     * @param apiBaseUrl base URI to the API, e.g.,
    ...
    public void login(String username, String password)
        ...
        login(username, password, false, this::confirmLogin);
    }
     ...
     * @param username    the name of the user to log in
     * @param password    the password of the user
     * @param resetTokens boolean determining whether to clear tokens and retrieve new ones
     * @throws LoginFailedException if the login failed for some reason
     */
    public void login(String username, String password, boolean resetTokens)
            throws LoginFailedException {
        login(username, password, resetTokens, this::confirmLogin);
    }
    ...
    public void clientLogin(String username, String password)
            throws LoginFailedException {
        login(username, password, false, this::confirmClientLogin);
    }
     ...
     * @throws LoginFailedException if the login failed for some reason
     */
    public void clientLogin(String username, String password, boolean resetTokens)
            throws LoginFailedException {
        login(username, password, resetTokens, this::confirmClientLogin);
    }
     ...
     * @throws LoginFailedException if the login failed for some reason
     */
    protected void login(String username, String password, boolean resetTokens, ILogin loginFunction) throws LoginFailedException {
        try {
            String loginTokenType = "login";
            if (resetTokens) {
                clearToken(loginTokenType);
            }
            String token = getOrFetchToken(loginTokenType);
            try {
                loginFunction.login(token, username, password);
            } catch (NeedLoginTokenException | TokenErrorException e) { // try once more
                clearToken(loginTokenType);
                token = getOrFetchToken(loginTokenType);
                loginFunction.login(token, username, password);
            }
     ...
     * @param username the name of the user that was logged in
     ...
     *
     * @param token    the login token string
     * @param username the name of the main user that was logged in
     * @param password the password used to log in
     * @throws IOException
     * @throws LoginFailedException
     */
